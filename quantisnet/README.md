...
```bash
docker container run -d -v NAME:/home/quantisnet/.quantisnetcore --name NAME quantisdev/quantisnet:latest
```
...
```bash
docker container run -d -v NAME:/home/quantisnet/.quantisnetcore --name NAME quantisdev/quantisnet:latest  -externalip=ADDRESS -masternode=1 -masternodeaddr=ADDRESS:9801 -masternodeprivkey=KEY -listen=1 -server=1
```
...
docker image pull quantisdev/quantisnet:latest
docker container run quantisdev/quantisnet:latest
```
...
The Docker image you want to use is _quantisdev/quantisnet:latest_.
...
services:
  NAME:
    ...
    healthcheck:
      ...
      timeout: 30s
      retries: 3
    image: quantisdev/quantisnet:latest
    networks:
      - NAME
    ports:
      - "ADDRESS:9801:9801"
